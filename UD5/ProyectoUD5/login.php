...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($user && password_verify($password, $user['password'])) {
        ...
        if (!isset($_COOKIE['login_count_' . $usuario])) {
            setcookie('login_count_' . $usuario, 1, time() + (86400 * 30), "/"); 
        } else {
            setcookie('login_count_' . $usuario, $_COOKIE['login_count_' . $usuario] + 1, time() + (86400 * 30), "/");
        }
